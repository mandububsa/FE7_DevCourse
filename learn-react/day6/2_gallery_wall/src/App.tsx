...
export default function App() {
  ...
  const [delIndex, setDelIndex] = useState<Set<number>>(new Set());
  const handleDelIndex = (index: number) => {
    setDelIndex((delIndex) => new Set([...delIndex, index]));
  };
  const hasDeletedImages = delIndex.size > 0;
  const handleRestoreAll = () => {
    setDelIndex(new Set());
  };
  const handleRestore = (index: number) => {
    setDelIndex((delIndex) => {
      const newSet = new Set(delIndex); // 새 껍데기
      newSet.delete(index);
      return newSet;
    });
  };
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 pb-8 sm:pb-12">
        ...
        >
          ...
          {galleryImages.map((image, index) => {
            const isDeleted = delIndex.has(index);
            return (
              <figure
                key={index}
                className={twMerge(
      ...
      <footer className="text-center py-6 sm:py-8 px-4 sm:px-6 border-t border-border bg-muted/30">
        <p className="text-muted-foreground text-sm sm:text-base">
          ...
          <span className="block mt-2 text-xs sm:text-sm">
            {delIndex.size} images hidden • {8 - delIndex.size} displayed
          </span>
        </p>
      </footer>
    </main>
