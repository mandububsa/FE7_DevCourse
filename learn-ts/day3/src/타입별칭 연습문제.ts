...
const createConfig = (config: Config): Config => config;

const config = createConfig({ host: "localhost", port: 8080, ssl: true });
...
const rectArea = calculateArea({ width: 10, height: 5 });
...
console.log(
  calculateAverageGrade({ name: "kim", age: 20, grades: [90, 80, 70, 80] })
);
...
function createResponse(res: Response): Response {
  return res;
}
console.log(
  createResponse({ status: "success", data: "John", message: "fetch success" })
...
function getDiscountedPrice(product: Product, discount: number) {
  if (product.inStock) return product.price * (100 - discount);
  return "sold";
}
console.log(
